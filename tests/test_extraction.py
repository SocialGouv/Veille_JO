...
class TestFonctionsPures(unittest.TestCase):
    def test_trouver_jo_par_date(self):
        conteneurs = [
            {"id": "JORFCONT2", "titre": "JO du 29/05/2026", "datePubli": _ms(date(2026, 5, 29))},
            {"id": "JORFCONT1", "titre": "JO du 28/05/2026", "datePubli": _ms(date(2026, 5, 28))},
        ]
    ...
    def test_trouver_jo_absent(self):
        ...
        self.assertIsNone(extraction.trouver_jo(conteneurs, date(2026, 5, 31)))  # dimanche
        ...
        arbre = {
            "structure": {
                "items": [
                    ...
                    {"sections": [
                        {"id": "JORFTEXT000054144858", "title": "Avis DABIGATRAN/OXAZEPAM"},
                        {"id": "SECTION1", "titre": "pas un texte"},
                        {"id": "JORFTEXT000054144856", "titre": "doublon à ignorer"},
                    ]},
                ]
            }
        }
        textes = extraction.lister_textes_sommaire(arbre)
        self.assertEqual([t[0] for t in textes],
                         ["JORFTEXT000054144856", "JORFTEXT000054144858"])
        self.assertEqual(textes[1][1], "Avis DABIGATRAN/OXAZEPAM")  # titre via « title »

    def test_extraire_champs_texte_filtre_et_ordonne(self):
        donnees = {
            "title": "x" * 30,                     # champ non ciblé : ignoré
            "content": "A" * 30,
            "annexes": [{"html": "B" * 25}, {"html": "B" * 25}],   # doublon conservé une fois
    ...
    def test_url_publique(self):
        self.assertEqual(extraction.url_publique("JORFTEXT000054144866"),
...
class TestClientPiste(unittest.TestCase):
    ...
    def test_retoken_sur_401(self, post, _sleep):
        ...
        post.side_effect = [
            ...
            self._reponse(json_donnees={"containers": [
                {"id": "JO1", "titre": "JO", "datePubli": _ms(date(2026, 5, 28))}]}),
        ...
        jetons = [appel.kwargs.get("headers", {}).get("Authorization")
                  for appel in post.call_args_list if "headers" in appel.kwargs]
        self.assertIn("Bearer T2", jetons)

    @mock.patch("extraction.time.sleep")
    @mock.patch("extraction.requests.post")
    def test_jo_introuvable_message_propre(self, post, _sleep):
        """Une date sans JO (dimanche) lève ErreurPiste avec un message exploitable."""
        post.side_effect = [
            self._reponse(json_donnees={"access_token": "T"}),
            self._reponse(json_donnees={"containers": [
                {"id": "JO1", "titre": "JO", "datePubli": _ms(date(2026, 5, 29))}]}),
        ]
        client = extraction.ClientPiste("id", "secret")
        with self.assertRaises(extraction.ErreurPiste) as contexte:
