...
from rapprochement import (SECTION_BAISSES, SECTION_EXTENSIONS, SECTION_HAUSSES,
                           ...
                           SECTION_RADIATIONS, consolider, mapper_laboratoire, nom_racine)
...
class TestNomRacine(unittest.TestCase):
    ...
    CAS = [
        ...
        ("FYCOMPA 2 mg, comprimé pelliculé x28", "FYCOMPA"),   # calibré 28/05 (cible : FYCOMPA)
        ...
        ("WEGOVY 0,25 mg FlexTouch (sémaglutide), solution injectable en stylo prérempli "
         ...
         "(B/1) + 4 aiguilles (laboratoires NOVO NORDISK)", "WEGOVY"),
        ("LIKOZAM 1 mg/ml (clobazam), suspension buvable flacon en verre jaune(brun) 150 ml "
         ...
         "(B/1) (laboratoires ADVICENNE)", "LIKOZAM"),
        ("MORPHINE (CHLORHYDRATE) 100 mg/5 ml, sans conservateur LAVOISIER, solution "
         "injectable, 5 ml en ampoule (B/10) (laboratoires CHAIX ET DU MARAIS)", "MORPHINE"),
        ("MORPHINE (CHLORHYDRATE) 20 mg/1 ml sans conservateur LAVOISIER, solution "
         "injectable,1 ml en ampoule (B/10) (laboratoires CHAIX ET DU MARAIS)", "MORPHINE"),
        ...
        ("SULFATE DE MAGNÉSIUM LAVOISIER", "SULFATE DE MAGNÉSIUM"),  # sel en tête : conservé
        ...
        ("ORACILLINE 500 000 UI/ 5 ml (phénoxyméthylpénicilline benzathine), suspension "
         "buvable, 120 ml en flacon", "ORACILLINE"),
        ...
        ("AZELASTINE CHLORHYDRATE/FLUTICASONE PROPIONATE BIOGARAN 137 MICROGRAMMES/ "
         "50 MICROGRAMMES", "AZELASTINE CHLORHYDRATE/FLUTICASONE PROPIONATE"),
        ...
        ("GARDASIL 9 0,5 ml, suspension injectable en seringue préremplie", "GARDASIL 9"),
        ...
        ("VITAMINE B12 1 000 µg/2 ml", "VITAMINE B12"),  # chiffre collé + vrais milliers
        ...
        ("HEPARINE CALCIQUE 12 500 UI/0,5 ml, solution injectable", "HEPARINE CALCIQUE"),
...
class TestMappingLaboratoires(unittest.TestCase):
    def test_mapping_unique_annexe_d(self):
        self.assertEqual(mapper_laboratoire("LAVOISIER"), "LAVOISIER - CHAIX ET DU MARAIS")
        self.assertEqual(mapper_laboratoire("CHAIX ET DU MARAIS"),
                         "LAVOISIER - CHAIX ET DU MARAIS")
    ...
    def test_laboratoire_colle_au_nom_des_generiques(self):
        ...
        self.assertEqual(nom_racine("MEROPENEM PANPHARMA 1 g, poudre pour perfusion"),
                         "MEROPENEM")
        ...
        self.assertEqual(nom_racine("PARACETAMOL TEVA PHARMA 500 mg, comprimé"),
                         "PARACETAMOL")
...
class TestInvariantOrdreDuMapping(unittest.TestCase):
    ...
    def test_racine_insensible_a_l_ordre_du_mapping(self):
        with mock.patch.dict(config.MAPPING_LABOS, self.MAPPING_TRIE, clear=True):
            self.assertEqual(nom_racine("ROSUVASTATINE VIATRIS SANTE 10 mg"), "ROSUVASTATINE")
    ...
    def test_conflit_de_suffixe_la_cle_la_plus_longue_gagne(self):
        with mock.patch.dict(config.MAPPING_LABOS, self.MAPPING_CONFLIT_SUFFIXE,
                             clear=True):
            self.assertEqual(nom_racine("LACOSAMIDE G.L. PHARMA 100 mg, comprimés "
                                        "pelliculés sécables (B/56)"), "LACOSAMIDE")
            self.assertEqual(mapper_laboratoire("G.L. PHARMA"), "G.L. PHARMA")
            self.assertEqual(mapper_laboratoire("PHARMA GENERIQUES SAS"),
                             "PHARMA GENERIQUE")
...
class TestConsolidation28Mai(unittest.TestCase):
    ...
    def test_8_lignes_une_par_racine(self):
        ...
        self.assertEqual(set(self.par_racine), {"WEGOVY", "MOUNJARO", "VGENFLI", "DABIGATRAN",
                                                "OXAZEPAM", "LIKOZAM", "MORPHINE", "FYCOMPA"})

    def test_sections_conformes(self):
        inscriptions = {l.racine for l in self.resultat.lignes_par_section(SECTION_INSCRIPTIONS)}
        self.assertEqual(inscriptions,
    ...
    def test_listes_et_liens_par_segment(self):
        ...
        for racine in ("WEGOVY", "MOUNJARO", "VGENFLI", "DABIGATRAN", "OXAZEPAM", "LIKOZAM"):
            ...
            self.assertEqual(ligne.listes,
                             [("SS", URL.format("JORFTEXT000054144800")),
                              ("Collectivité", URL.format("JORFTEXT000054144802"))], racine)

    def test_liens_prix_annexe_e(self):
        attendus = {
            "WEGOVY": "JORFTEXT000054144866", "MOUNJARO": "JORFTEXT000054144870",
            "VGENFLI": "JORFTEXT000054144856", "DABIGATRAN": "JORFTEXT000054144858",
            "OXAZEPAM": "JORFTEXT000054144858",            # avis partagé DABIGATRAN/OXAZEPAM
            "LIKOZAM": "JORFTEXT000054144862", "MORPHINE": "JORFTEXT000054144874",
    ...
    def test_taux_des_decisions_uncam(self):
        ...
        attendus = {"WEGOVY": "JORFTEXT000054144868", "MOUNJARO": "JORFTEXT000054144872",
    ...
    def test_laboratoires_mappes(self):
        attendus = {
            "WEGOVY": "NOVO NORDISK", "MOUNJARO": "LILLY", "VGENFLI": "FRESENIUS KABI",
            "DABIGATRAN": "TEVA SANTE", "OXAZEPAM": "ARROW", "LIKOZAM": "ADVICENNE",
    ...
    def test_indications_de_section_recopiees(self):
        self.assertTrue(self.par_racine["WEGOVY"].indication.startswith("Chez l'adulte"))
        ...
        self.assertEqual(self.par_racine["MOUNJARO"].indication, "à compléter manuellement")
    ...
    def test_aucune_ligne_a_verifier_ni_anomalie(self):
        ...
        self.assertEqual({l.racine for l in self.resultat.lignes if l.a_verifier}, set())
        self.assertEqual(self.resultat.anomalies, [])

    def test_sources_traceables(self):
        self.assertEqual(self.par_racine["MORPHINE"].sources,
                         [("JORFTEXT000054144874", "avis de hausse de prix")])


def _texte(type_texte, produits, id_="JORFTEXT000054140000", ambigu=False, listes=(),
           prix_deux_sens=False):
    return TexteAnalyse(id=id_, url=URL.format(id_), titre="titre", type_texte=type_texte,
                        ambigu=ambigu, produits=produits, texte_nettoye="corps",
                        listes=list(listes), prix_deux_sens=prix_deux_sens)
...
class TestMultiLaboratoires(unittest.TestCase):
    ...
    def test_une_ligne_par_laboratoire(self):
        ...
        textes = [_texte("avis_hausse_prix",
    ...
    def test_contribution_sans_laboratoire_propagee(self):
        ...
        textes = [
            _texte("arrete_inscription",
                   [ProduitExtrait("PRODUIT X 10 mg", "")], listes=["SS"]),
            _texte("avis_hausse_prix",
...
class TestCinqListes(unittest.TestCase):
    def test_ordre_et_liens_des_listes(self):
        ...
        textes = [
            _texte("arrete_inscription", produit, id_="JORFTEXT000054140010",
                   listes=["Rétrocession"]),
            _texte("arrete_inscription", produit, id_="JORFTEXT000054140011",
                   listes=["SS"]),
            _texte("arrete_inscription", produit, id_="JORFTEXT000054140012",
                   listes=["LES SMR"]),
        ]
        resultat = consolider(textes, DATE_JO)
        (ligne,) = resultat.lignes
...
class TestNouvellesSections(unittest.TestCase):
    def test_radiation(self):
        ...
        textes = [_texte("arrete_radiation",
        ...
        self.assertEqual(ligne.listes_radiation,
                         [("LES MCO", URL.format("JORFTEXT000054140000"))])

    def test_modification_libelle(self):
        textes = [_texte("modification_libelle",
    ...
    def test_modification_libelle_transfert_d_exploitation(self):
        ...
        textes = [_texte("modification_libelle",
                         [ProduitExtrait("GEMZAR 1000 mg, poudre pour perfusion",
                                         "CHEPLAPHARM FRANCE",
    ...
    def test_transfert_vers_le_meme_laboratoire_sans_fleche(self):
        ...
        textes = [_texte("modification_libelle",
                         [ProduitExtrait("MELATONINE ARROW LP 2 mg, comprimé", "ARROW",
    ...
    def test_regle_sirturo(self):
        ...
        textes = [
            _texte("arrete_inscription", produit, id_="JORFTEXT000054140020",
                   listes=["Rétrocession"]),
            _texte("extension_indication",
                   [ProduitExtrait("SIRTURO 100 mg, comprimé", "JANSSEN-CILAG",
    ...
    def test_extension_seule_sans_regroupement(self):
        ...
        textes = [_texte("extension_indication",
                         [ProduitExtrait("PRODUIT Z 5 mg", "LABO",
                                         indication="Nouvelle indication")])]
        resultat = consolider(textes, DATE_JO)
        (ligne,) = resultat.lignes
        self.assertEqual(ligne.section, SECTION_EXTENSIONS)
        self.assertEqual(ligne.listes, [])
...
class TestRapprochementParCip(unittest.TestCase):
    ...
    def test_avis_abrege_rattache_a_l_arrete(self):
        ...
        textes = [
            _texte("arrete_inscription",
                   [ProduitExtrait("MEROPENEM 1 g, poudre pour solution injectable",
                                   "PANPHARMA", cip="3400930111111",
                                   indication="Idem MERONEM")],
                   id_="JORFTEXT000054140030", listes=["LES SMR"]),
            _texte("avis_prix",
                   [ProduitExtrait("MEROPENEM PAN 1G INJ FL", "PANPHARMA",
                                   cip="3400930111111")],
                   id_="JORFTEXT000054140031"),
        ]
        resultat = consolider(textes, DATE_JO)
        (ligne,) = resultat.lignes                      # une seule ligne, pas de parasite
        self.assertEqual(ligne.produit, "MEROPENEM")    # la dénomination de l'arrêté fait foi
        self.assertEqual(ligne.section, SECTION_INSCRIPTIONS)
        self.assertEqual(ligne.lien_prix, URL.format("JORFTEXT000054140031"))
        self.assertEqual(resultat.anomalies, [])        # avis rattaché : plus d'« à vérifier »

    def test_avis_en_premier_dans_le_sommaire(self):
        """L'élection de la clé canonique ne dépend pas de l'ordre du sommaire."""
        textes = [
            _texte("avis_prix",
                   [ProduitExtrait("ERIBULINE HIK FL", "HIKMA",
                                   cip="3400930222222")],
                   id_="JORFTEXT000054140032"),
            _texte("arrete_inscription",
                   [ProduitExtrait("ERIBULINE HIKMA 0,44 mg/ml, solution injectable",
                                   "HIKMA FRANCE", cip="3400930222222")],
                   id_="JORFTEXT000054140033", listes=["LES MCO"]),
        ]
        resultat = consolider(textes, DATE_JO)
        (ligne,) = resultat.lignes
    ...
    def test_deux_arretes_meme_code_racine_courte_elue(self):
        ...
        textes = [
            _texte("arrete_inscription",
                   ...
                   id_="JORFTEXT000054140035", listes=["Rétrocession"]),
            _texte("arrete_inscription",
                   [ProduitExtrait("POMALIDOMIDE LUPIN 4 mg, gélule", "MEDISOL", cip=cip)],
                   id_="JORFTEXT000054140036", listes=["LES SMR"]),
            _texte("arrete_inscription",
                   [ProduitExtrait("POMALIDOMIDE LUPIN 4 mg, gélule", "MEDISOL", cip=cip)],
                   id_="JORFTEXT000054140037", listes=["Collectivité"]),
        ]
        resultat = consolider(textes, DATE_JO)
        (ligne,) = resultat.lignes
    ...
    def test_sans_cip_comportement_inchange(self):
        ...
        textes = [
            _texte("arrete_inscription",
                   [ProduitExtrait("PRODUIT K 5 mg, comprimé", "LABO")], listes=["SS"]),
            _texte("avis_prix", [ProduitExtrait("PRODUIT K ABR CPR", "LABO")],
...
class TestTauxDeParticipation(unittest.TestCase):
    ...
    def _inscription(self, cip="3400930333211"):
        return _texte("arrete_inscription",
                      [ProduitExtrait("ATIMIAC 20 mg/5 mg par ml, collyre en solution",
                                      "HORUS PHARMA", cip=cip, indication="Idem AMIKACINE")],
                      id_="JORFTEXT000054457332", listes=["SS"])

    def test_taux_rattache_a_l_inscription_par_le_cip(self):
        textes = [self._inscription(),
                  _texte("decision_taux",
                         [ProduitExtrait("ATIMIAC 20 mg/5 mg par ml (dorzolamide, timolol), "
                                         "collyre", "HORUS PHARMA", cip="3400930333211",
                                         taux="0.35")],
    ...
    def test_taux_divergents_entre_presentations_signales(self):
        ...
        textes = [self._inscription(),
                  _texte("decision_taux",
                         [ProduitExtrait("ATIMIAC 20 mg, collyre", "HORUS PHARMA",
                                         cip="3400930333211", taux="0.35"),
                          ProduitExtrait("ATIMIAC 40 mg, collyre", "HORUS PHARMA",
                                         cip="3400930333211", taux="0.65")],
                         id_="JORFTEXT000054458542")]
        resultat = consolider(textes, DATE_JO)
        (ligne,) = resultat.lignes
        self.assertEqual(ligne.taux, "0.35")
        self.assertTrue(ligne.a_verifier)
        self.assertTrue(any("35% vs 65%" in motif for motif in ligne.motifs_verification),
    ...
    def test_decision_sans_taux_lisible_signalee(self):
        ...
        self.assertTrue(any("valeur non extraite" in motif
...
class TestExtensionParArrete(unittest.TestCase):
    ...
    def test_extension_avec_listes_reste_en_extensions(self):
        ...
        produit = ProduitExtrait("ACETATE DE CYPROTERONE ARROW 50 mg, comprimé", "ARROW",
                                 indication="Traitement pour la réduction…")
        textes = [
            _texte("extension_indication", [produit],
                   id_="JORFTEXT000054140040", listes=["SS"]),
            _texte("extension_indication", [produit],
                   id_="JORFTEXT000054140041", listes=["Collectivité"]),
        ]
        resultat = consolider(textes, DATE_JO)
        (ligne,) = resultat.lignes
        self.assertEqual(ligne.section, SECTION_EXTENSIONS)
...
class TestOrientationParReferentiel(unittest.TestCase):
    ...
    def _avis(self, ppttc="156,38 €", cip="3400930084939"):
        return _texte("avis_prix",
                      [ProduitExtrait("DARUNAVIR VIATRIS 400 mg, comprimé", "VIATRIS",
                                      cip=cip, ppttc=ppttc)])
    ...
    def test_baisse_constatee(self):
        ...
        self.assertEqual(referentiel.enregistres,
                         [("3400930084939", DATE_JO, self._D("156.38"))])
    ...
    def test_prix_inconnu_ou_egal_reste_a_verifier(self):
        for referentiel in (_ReferentielFactice({}),                       # inconnu
    ...
    def test_sens_contradictoires_jamais_devines(self):
        ...
        referentiel = _ReferentielFactice({"3400930000001": self._D("10.00"),
                                           "3400930000002": self._D("30.00")})
        textes = [_texte("avis_prix",
                         [ProduitExtrait("PRODUIT M 5 mg, comprimé", "LABO",
                                         cip="3400930000001", ppttc="12,00 €"),
                          ProduitExtrait("PRODUIT M 10 mg, comprimé", "LABO",
                                         cip="3400930000002", ppttc="25,00 €")])]
    ...
    def test_avis_annoncant_les_deux_sens_jamais_tranche_par_le_referentiel(self):
        ...
        """
        avis = _texte("avis_prix",
                      [ProduitExtrait("MORPHINE (CHLORHYDRATE) LAVOISIER 10 mg/ml, "
                                      "solution injectable", "LAVOISIER",
                                      cip="3400930084939", ppttc="3,10 €")],
                      prix_deux_sens=True)     # les deux sens annoncés (analyse.py)
        referentiel = _ReferentielFactice({"3400930084939": self._D("4.00")})
        resultat = consolider([avis], DATE_JO, referentiel=referentiel)
        (ligne,) = resultat.lignes
        self.assertEqual(ligne.section, SECTION_HAUSSES)   # convention, jamais « baisse »
        self.assertTrue(ligne.a_verifier)
        self.assertTrue(any("majoration" in motif and "baisse" in motif
                            for motif in ligne.motifs_verification),
                        ligne.motifs_verification)
        self.assertTrue(any(anomalie.startswith("MORPHINE : à vérifier —")
                            for anomalie in resultat.anomalies), resultat.anomalies)
    ...
    def test_bascule_orientation_desactivee_repli_a_verifier(self):
        ...
        """
        self.assertIn("ORIENTATION_PRIX_AUTO", vars(config))   # la bascule existe toujours
        avis = self._avis()
        referentiel = _ReferentielFactice({"3400930084939": self._D("160.24")})
        ...
        self.assertTrue(any("sens à vérifier" in motif for motif in sans.motifs_verification))
        self.assertTrue(any("DARUNAVIR" in anomalie for anomalie in resultat_sans.anomalies))
        # « sans autre changement de rendu » : produit, laboratoire et lien identiques.
        self.assertEqual((sans.produit, sans.laboratoire, sans.lien_prix),
    ...
    def test_avis_a_deux_sens_ne_deborde_pas_sur_les_lignes_qu_il_ne_classe_pas(self):
        ...
        """
        produit = [ProduitExtrait("INSCRIVIN 10 mg, comprimé", "LABO INSCRIT",
                                  cip="3400930084939", indication="Asthme sévère")]
        avis_contradictoire = _texte("avis_prix",
                                     [ProduitExtrait("INSCRIVIN 10 mg, comprimé",
                                                     "LABO INSCRIT",
                                                     cip="3400930084939",
                                                     ppttc="8,50 €")],
                                     id_="JORFTEXT000054140031", prix_deux_sens=True)
        referentiel = _ReferentielFactice({"3400930084939": self._D("9.00")})

        # a. section décidée par l'inscription : le prix n'y décide rien.
        inscription = _texte("arrete_inscription", produit, id_="JORFTEXT000054140030",
                             listes=["SS"])
        (ligne,) = consolider([inscription, avis_contradictoire], DATE_JO,
                              referentiel=referentiel).lignes
        self.assertEqual(ligne.section, SECTION_INSCRIPTIONS)
        ...
        extension = _texte("extension_indication",
                           [ProduitExtrait("INSCRIVIN 10 mg, comprimé", "LABO INSCRIT",
                                           cip="3400930084939",
                                           indication="Extension à l'enfant")],
                           id_="JORFTEXT000054140032")
        resultat = consolider([inscription, extension, avis_contradictoire], DATE_JO,
    ...
    def test_avis_deja_oriente_par_son_texte_inchange(self):
        ...
        referentiel = _ReferentielFactice({"3400930084939": self._D("150.00")})  # dirait hausse
        textes = [_texte("avis_baisse_prix",
                         [ProduitExtrait("DARUNAVIR VIATRIS 400 mg, comprimé", "VIATRIS",
                                         cip="3400930084939", ppttc="156,38 €")])]
        resultat = consolider(textes, DATE_JO, referentiel=referentiel)
...
class TestCasLimites(unittest.TestCase):
    ...
    def test_decision_taux_isolee_signale_sans_ligne(self):
        textes = [_texte("decision_taux",
                         [ProduitExtrait("PRODUIT Y", "LABO")])]
        resultat = consolider(textes, DATE_JO)
        self.assertEqual(resultat.lignes, [])              # aucune section applicable
        self.assertTrue(any("PRODUIT Y" in a for a in resultat.anomalies))  # jamais silencieux

    def test_indication_manquante_a_completer(self):
        """Inscription sans indication de section : jamais de vide silencieux."""
        textes = [_texte("arrete_inscription",
                         [ProduitExtrait("PRODUIT Z 5 mg", "LABO")], listes=["SS"])]
        resultat = consolider(textes, DATE_JO)
    ...
    def test_indication_de_section_conservee(self):
        ...
        textes = [_texte("arrete_inscription",
                         [ProduitExtrait("PRODUIT W 5 mg", "LABO",
    ...
    def test_indications_divergentes_toutes_conservees(self):
        ...
        textes = [_texte("arrete_inscription",
                         [ProduitExtrait("FULBEV 25 mg/ml, solution", "LABO",
                                         indication="Indication A"),
                          ProduitExtrait("FULBEV 100 mg/4 ml, solution", "LABO",
                                         indication="Indication B"),
                          ProduitExtrait("FULBEV 400 mg/16 ml, solution", "LABO",
                                         indication="Indication A")],
                         listes=["SS"])]
        resultat = consolider(textes, DATE_JO)
        (ligne,) = resultat.lignes
    ...
    def test_arrete_ambigu_signale(self):
        ...
        textes = [_texte("arrete_inscription",
                         [ProduitExtrait("PRODUIT V 5 mg", "LABO")], ambigu=True)]
        resultat = consolider(textes, DATE_JO)
        (ligne,) = resultat.lignes
        self.assertEqual(ligne.section, SECTION_INSCRIPTIONS)
        self.assertTrue(ligne.a_verifier)
...
class TestFusionDesContributionsCommunes(unittest.TestCase):
    ...
    def _cumul(self, type_texte, id_, listes=(), indication="", ambigu=False):
        ...
        rapprochement._accumuler(cumul, _texte(type_texte, [produit], id_=id_,
                                               listes=listes, ambigu=ambigu), produit)
        return cumul
    ...
    def test_drapeaux_cumules_liens_du_laboratoire_conserves(self):
        ...
        commun = self._cumul("arrete_inscription", "JORFTEXT000000000002",
                             listes=["SS", "Collectivité"])
        ...
        self.assertEqual(ligne.inscription.listes["SS"], URL.format("JORFTEXT000000000001"))
        self.assertEqual(ligne.inscription.listes["Collectivité"],
                         URL.format("JORFTEXT000000000002"))
    ...
    def test_tracabilite_ordre_preserve_sans_doublon(self):
        ligne = self._cumul("arrete_inscription", "JORFTEXT000000000001",
                            listes=["SS"], indication="Indication A")
        commun = self._cumul("extension_indication", "JORFTEXT000000000002",
                             indication="Indication B", ambigu=True)
        commun.signaler("motif du commun")
        ligne.signaler("motif de la ligne")
        ligne.fusionner_contributions_communes(commun)
        self.assertEqual(ligne.tracabilite.indications, ["Indication A", "Indication B"])
        self.assertEqual(ligne.tracabilite.sources,
                         [("JORFTEXT000000000001", "arrêté d'inscription"),
                          ("JORFTEXT000000000002", "extension d'indication")])
        self.assertEqual(ligne.tracabilite.motifs,
                         ["motif de la ligne",
    ...
    def test_fusion_idempotente(self):
        ...
        ligne = self._cumul("arrete_inscription", "JORFTEXT000000000001", listes=["SS"])
        commun = self._cumul("avis_prix", "JORFTEXT000000000002", indication="Indication B")
        ligne.fusionner_contributions_communes(commun)
        attendu = (dict(ligne.inscription.listes), ligne.prix.lien,
                   list(ligne.tracabilite.indications), list(ligne.tracabilite.sources),
                   list(ligne.tracabilite.motifs))
        ligne.fusionner_contributions_communes(commun)
        self.assertEqual((dict(ligne.inscription.listes), ligne.prix.lien,
                          list(ligne.tracabilite.indications),
                          list(ligne.tracabilite.sources),
                          list(ligne.tracabilite.motifs)), attendu)
...
class TestSectionDuCumulEstPure(unittest.TestCase):
    ...
    def test_avis_non_oriente_retourne_le_motif_sans_signaler(self):
        ...
        self.assertEqual(motif, f"avis de prix non orienté ({cumul.prix.lien}) : classé en "
    ...
    def test_section_orientee_sans_motif(self):
        ...
        self.assertEqual(rapprochement._section_du_cumul(cumul), (SECTION_BAISSES, None))
...
class TestOrdreDeSortie(unittest.TestCase):
    ...
    def test_anomalies_sans_section_avant_le_recapitulatif(self):
        ...
        textes = [_texte("avis_prix", [ProduitExtrait("DOUTEUX 10 mg", "LABO")],
                         id_="JORFTEXT000000000001"),
                  _texte("decision_taux", [ProduitExtrait("TAUX SEUL 5 mg", "LABO")],
                         id_="JORFTEXT000000000002")]
        resultat = consolider(textes, DATE_JO)
        self.assertEqual(len(resultat.anomalies), 2)
        self.assertTrue(resultat.anomalies[0].startswith("TAUX SEUL : vu uniquement dans"))
    ...
    def test_recapitulatif_une_entree_par_racine_et_non_par_laboratoire(self):
        ...
        textes = [_texte("avis_prix",
                         [ProduitExtrait("GENERIQUE 10 mg, comprimé", labo)
                          for labo in labos])]
        resultat = consolider(textes, DATE_JO)
